...
const getItem = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        for (let i = 0; i < (item.list as List).users.length; i++) {
            if ((item.list as List).users[i].toString() === (user._id as ObjectId).toString()) {
                found = true;
                break;
            }
        }
...
const updateItem = asyncHandler(
    async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (req.file) {
            ...
            if (item.img) {
                const [_, image_url ] = await Promise.all([deleteImage(item.img), uploadImage(req.file, id)]);
                item.img = image_url;
            } else{
                item.img = await uploadImage(req.file, id);
            }
        }
        item.name = name;
