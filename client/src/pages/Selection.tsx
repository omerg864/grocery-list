...
import {
	...
	ITEM_SELECTION_LIMIT,
} from '../utils/requestsConst';
...
function Selection() {
	...
	const [updatedBundles, setUpdatedBundles] =
		useRecoilState<Date>(updatedBundlesAtom);
			...
			(data) => {
				let itemsTemp: Item[] = data.items.map((item: Item) => ({
					...
					imageMemo: (
						<MemoizedImage
							className="item-img"
							src={item.img ? item.img : '/item.png'}
							alt={item.name}
						/>
					),
				}));
	...
	const getAll = async () => {
		...
		if (
			...
			getMinutesBetweenDates(itemsData.updated, new Date()) > 10
		) {
			promises.push(getItems());
		}
		if (
			bundles.length === 0 ||
			getMinutesBetweenDates(updatedBundles, new Date()) > 10
		) {
	...
	return (
		<main>
			...
			<ItemsList
				items={itemsData.items.slice(0, ITEM_SELECTION_LIMIT)}
			...
			<BundleList
				bundles={bundles.slice(0, BUNDLE_SELECTION_LIMIT)}
				onItemClick={onBundleClicked}
			/>
		</main>
	);
}
