	...
	return (
		<div className="items-list">
			{props.items.length === 0 ? (
				...
				props.items.map((item: Item | ListItem, index) => (
					<motion.div
						initial={{ x: dimensions.width + 200 + index * 80 }}
						animate={{ x: 0 }}
						...
						{props.amounts && (
							...
							>
								{item.unit &&
									...
									(item.unit === 'pc' ? (
										<Counter
											...
											addCounter={() =>
												props.addCounter!(item._id)
											}
											removeCounter={() =>
												props.removeCounter!(item._id)
											}
											count={
												props.amounts?.find(
													(obj) => obj.id === item._id
								...
								<FormControl fullWidth>
									<InputLabel color="success" id="unit-label">
										{t('unit')}
									</InputLabel>
									<Select
										labelId="unit-label"
